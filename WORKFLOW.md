...
The importable client artifact is a small Prism bootstrap generated by
`scripts/build-client-bundle.sh`. It contains the instance metadata and Packwiz
Installer bootstrap rather than a duplicate copy of every mod. On first launch,
and before every later launch, it synchronizes managed files from the stable
`main/pack.toml` URL. Prism should use Java 17, a 2–8 GiB heap, and G1. This
matches Forge 1.20.1's expected runtime and avoids the native Oracle Java 21
failures observed on Apple Silicon.
